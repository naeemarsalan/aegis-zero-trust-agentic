...
| T2 | Delegated read — downstream sees the **user** | ✅ | **re-proven 2026-06-28: READ → 200** (real pfSense rules) with the ext-proc `VAULT_ADDR` in-cluster fix + a fresh consent grant. | — (please confirm) |
| T3 | Write blocked by default (fail-closed) | ✅ | **re-proven 2026-06-28: WRITE → 403 `grant_scope_denied`** (read-only grant denies the write). | — (please confirm) |
| T4 | Human approves to elevate; **approver ≠ requester** (SoD) | ✅ | **re-proven 2026-06-28: mint → 200** (approver `arsalan-approver`, Gitea PR #81); **clean SoD test: self-approve → 403**. (jit-approver `VAULT_ADDR` also re-fixed in-cluster — the mint's Vault credential-issuance was hitting the degraded external route.) | — (please confirm) |
| T5 | JIT short-lived scoped capability, auto-expires (write 5m single-use / exec 30m) | ✅ | **re-proven 2026-06-28: elevated write → 200** with a freshly minted capability JWT (912 B, `expires_at` +10m). *Note:* the gateway authorized the elevated write (200) but the specific pfSense rule wasn't separately re-confirmed this run (cf. id=52 on 2026-06-25). | — (please confirm) |
