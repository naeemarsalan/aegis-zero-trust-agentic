...
[.lead]
Every component, which cluster it runs on, and the primary communication paths for the Aegis zero-trust agentic platform.
...
. **Kyverno ext_authz** decides allow/deny — it cannot mutate.
. **ext-proc-delegation** mutates headers — it cannot grant access.
...
| SPIRE server, SPIRE agent, SPIFFE CSI Driver, OIDC discovery
| Default deny; SPIRE agent egress to server:8081; OIDC Route ingress from router

| `keycloak`
| RHBK, CNPG Postgres
| Default deny; ingress 8443 from router; egress CNPG:5432, SPIRE OIDC:443

| `vault`
| Vault (raft), Vault Agent Injector
| Default deny; ingress 8200 from `mcp-gateway`, `keycloak`, `agentic-mcp`, `agent-sandbox`; Route ingress for CLI

| `kyverno`
| Kyverno admission, kyverno-authz-server
| Default deny; ingress 9081 from `mcp-gateway`; admission webhook from API server

| `mcp-gateway`
| agentgateway, ext-proc-delegation, jit-approver
| Default deny; ingress 443 from router; ext-proc:9000 from agentgateway; jit:8080 from ext-proc; webhook Route for Gitea

| `agentic-mcp`
| pfsense-mcp
| Default deny; ingress 8000 from `mcp-gateway` only

| `agent-sandbox`
| agent pods (Kata)
| Default deny; egress to `mcp-gateway.apps.anaeem.na-launch.com:443` only

| `agentic-observability`
| OTel collector, AlertManager
| Default deny; ingress OTLP from platform namespaces; egress to Loki:3100, EDA:443
|===

== Cross-references

* xref:04-uc1-credential-delegation.adoc[UC1 sequence diagram] — delegated tool call, step by step
* xref:05-uc2-jit-sub-identity.adoc[UC2 sequence diagram] — JIT approval flow, step by step
* xref:11-security-model.adoc[Security model & trust boundaries] — STRIDE per hop
* xref:20-components.adoc[Component pages] — per-component placement, interfaces, and verify commands
