...
:navtitle: ext-proc-delegation

[.lead]
`ext-proc-delegation` is **the only custom-built component** in the Aegis platform.
It is a Go service implementing Envoy's gRPC External Processing (`ext_proc`) protocol.
It runs as a sidecar alongside agentgateway and intercepts every MCP tool-call request in the gateway pipeline.

For each request it:

. Parses the agent's identity claims from gateway metadata (`dev.agentgateway.jwt`)
. Extracts the tool name and arguments from the MCP JSON-RPC body
. Executes a two-leg Keycloak token exchange (RFC 7523 → RFC 8693) to produce a user-scoped downstream token
. Authenticates to Vault using its own SPIFFE SVID and reads the per-tool secret
. Injects the user token and the per-tool secret into the forwarded request headers
. Clears the agent's original credential from the request
. On the response leg, strips all credential and authorization headers
. Emits a SHA-256-hashed-args audit event and an OTel span
...
| Image
| `oci.arsalan.io/nvidia-ida/ext-proc-delegation:dev`

| Internal service
| `ext-proc-delegation.mcp-gateway.svc.cluster.local:9000` (gRPC)

| External exposure
| None — internal only; never exposed via Route
...
The service implements a `mode` flag (xref:52-adr-keycloak-token-exchange-strategy.adoc[ADR 0003]) for resilience against RHBK preview-feature instability:
...
| `standard`
| RFC 7523 jwt-bearer (SVID → Keycloak realm token, RHBK preview) → RFC 8693 exchange (→ downstream audience)

| `legacy`
| Fallback path that avoids the fragile RFC 7523 preview leg; uses a pre-provisioned federated-identity arrangement
|===

The active mode is selected per environment in the kustomize overlay config.
Both modes emit an audit attribute recording which mode produced each token — observable per request in Loki.
...
| inbound
| 9000 gRPC
| ext_proc request/response processing stream

| kyverno-authz-server
| outbound
| 9081 gRPC
| Policy evaluation for each tool call

| Keycloak
| outbound
| 443 HTTPS
| RFC 7523 + RFC 8693 token exchange

| Vault
| outbound
| 8200 HTTPS
| SVID login + per-tool secret read

| jit-approver
| outbound
| 8080 HTTP
| JIT hold: pause request pending UC2 approval

| Loki / OTel Collector
| outbound
| 4317 gRPC
| Hashed-args audit event + OTel span
|===
