...
. Opens a **Gitea pull request** carrying the grant manifest and justification
. Waits for a human to merge the PR (the approval act)
. Verifies the merge webhook (HMAC + repo allowlist)
. Re-reads the scope from the committed manifest and re-validates it
. Calls the Vault Kubernetes secrets engine to mint an ephemeral SA + Role + RoleBinding
. Mints a signed RS256 **session-capability JWT** for the Kyverno gate
...
. Posts a summary PR comment and emits an OTel span when the session closes
...
| Webhook Route
| OCP Route exposing `:9443` for inbound Gitea merge webhooks only
...
| Gitea repo
| `anaeem/nvidia-ida` at `https://git.arsalan.io`
|===
...
A missing, invalid, or expired JWT → 403.
...
| outbound
| 443 HTTPS
| PR creation for JIT approval requests

| Gitea webhook
| inbound
| 9443 HTTPS (Route)
| Merge event callback — release held session

| Vault
| outbound
| 8200 HTTPS
| Kubernetes engine + KV write + HMAC secret read
...
WARNING: `jit-approver` is a bespoke Python service.
The Gitea PR-merge-as-approval pattern is a custom integration.
Consider it PoC-grade.
