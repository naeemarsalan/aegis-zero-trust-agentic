...
:navtitle: agentgateway

[.lead]
agentgateway is the MCP-protocol-aware ingress and policy enforcement plane.
Every AI agent call to a downstream MCP server transits the gateway, which:

. **Validates** the bearer JWT-SVID against the Keycloak JWKS (authentication)
. **Calls Kyverno** ext_authz for a per-tool allow/deny decision (authorization)
. **Calls ext-proc-delegation** ext_proc to swap the agent identity for the user's credential (mutation)
. **Proxies** the request with the verified user identity to the downstream MCP server
. **Strips** credential headers from the response before returning it to the agent
...
| Internal service
| `agentgateway.mcp-gateway.svc.cluster.local:8080`

| MCP transport
| StreamableHTTP (primary); SSE fallback for older agent SDKs
|===
...
Both filters (`extAuthz` and `extProc`) are configured as **required**.
If either is unreachable or returns an error, the gateway returns 503 to the agent.
There is no fallback path that allows a request to proceed without both checks.
...
| outbound
| 9000 gRPC
| ext_proc header mutation filter
...
| outbound
| 8080 HTTP
| JIT approval gate polling

| pfsense-mcp (agentic-mcp)
| outbound
| 8000 HTTP
| Proxied MCP server calls
...
WARNING: agentgateway is at an **alpha** release stage — Red Hat has not committed to a stable API surface.
Field names, CRD schema, and default behaviors may change between builds.
The deployment pins the Helm chart version to guard against unexpected CRD churn.
