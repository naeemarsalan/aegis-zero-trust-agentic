...
. **Kubernetes admission controller** — enforces baseline security invariants cluster-wide: no privileged containers, required SPIFFE labels, no `automountServiceAccountToken` on agent pods, and OpenShell sandbox admission guardrails. `failurePolicy: Fail` on all webhooks — if Kyverno is down, new resource creation is blocked.
